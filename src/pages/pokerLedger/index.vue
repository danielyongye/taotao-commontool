      ...
      <template v-if="bottomTab === 'stats'">
          ...
          >
            <view class="record-head">
              <view class="record-head-left">
                ...
                <view v-if="item.hasStakes" class="stakes-chip">
                  <text class="stakes-chip-text">{{ item.stakesText }}</text>
                </view>
            ...
            <view v-if="item.hasDetail" class="record-meta">
              <u-icon name="clock" size="13" color="#A1A8B8" />
              <text class="record-meta-text">{{ item.detailLine }}</text>
            </view>

            <view v-else class="record-meta">
              <text class="record-meta-text record-meta-text--muted">暂无时长与盲注</text>
            </view>
          </view>
        </view>
        ...
        <view v-else class="empty-card">
          <view class="empty-icon-wrap">
            <u-icon name="grid-fill" size="32" color="#6366F1" />
          </view>
          <text class="empty-title">还没有牌局记录</text>
          <text class="empty-desc">记下每一局盈亏，盲注和时长也一目了然</text>
          <view
            class="empty-btn"
...
.page-bg {
  ...
  top: 0;
  height: 420rpx;
  background: linear-gradient(180deg, #dce8ff 0%, #e8e4ff 42%, #f7f8fc 100%);
  pointer-events: none;
  z-index: 0;
}
